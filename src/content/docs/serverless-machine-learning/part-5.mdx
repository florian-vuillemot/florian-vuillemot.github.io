---
title: Part 5 - Increasing the model size
description: Using a File Share to increase the model size.
---

# Introduction
The [previous article](./part-4) allows deployment rollback improving reliability in case of deployment failure. Unfortunately, each deployment impacts the application's reliability and contains a risk, especially in our case because we are training a model that can rely on a random state. Besides, its size is limited because the model is in the artifact and mixed with the application code.
...
# Current status
The [previous article](./part-4) leads to these files:
...
![Create Storage Account](../../../assets/serverless-machine-learning/part-5/create-storage-account.gif)

Now let's create the File Share **models**:

![Create File Share](../../../assets/serverless-machine-learning/part-5/create-file-share.gif)
...
![Update permission](../../../assets/serverless-machine-learning/part-5/file-share-permission.gif)
...
> Blue-green deployment with Azure Function is our context explained [here](./part-4).
...
![Create environment variables](../../../assets/serverless-machine-learning/part-5/create-env-var.gif)
